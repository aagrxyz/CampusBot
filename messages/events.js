...
function make_story(events)
{
    ...
    for(var i=0;i<events.length && i< 6;i++)
    {
        ...
        ans.push(event.name+"\n\n"+start.toLocaleDateString('en-US',{weekday: "short", day: "2-digit", month: "short",year: "numeric",timeZone: "Asia/Kolkata"})+": "+start.toLocaleTimeString('en-US',{timeZone: 'Asia/Kolkata'})+" - "+end.toLocaleTimeString('en-US',{timeZone: 'Asia/Kolkata'})+"\n\n"+event.place.name+"\n\n"+"Link- facebook.com/events/"+event.id);
    }
    return ans;
}
