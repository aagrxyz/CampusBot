...
bot.dialog('/main',[
    ...
    function(session,args,next) {
        ...
        // console.log(intents);
        builder.Prompts.choice(session, "What would you like to get (type end to quit)?", "Upcoming Events|Class Schedule|Papers Download|Who is|Mess Schedule|Exam Schedule|Course Review|FAQ|Complaint|Help");
    },
        ...
        if (results.response) {
            ...
            for(var i=0;i<courses_to_msg.length;i++)
            {
            	...
            	if(schedule.iscoursePresent(session.userData.en,courses_to_msg[i]))
            	{
					...
					var examDate = new Date("2017-03-28 GMT+0530");
	        	...
	        	{
	        		session.send("You have not enrolled for " + courses_to_msg[i]);
	        	}
